...
const PROJECTS = [
  {
    name: "Around Within",
    stack: ["React", "Node", "Express", "MongoDB", "Twilio"],
    blurb: "Mental-healthcare platform for booking therapy and consultations with secure auth and SMS verification.",
    color: "var(--accent-yellow)",
  },
  {
    name: "La Roche University",
    stack: ["WordPress", "Jenzabar", "WCAG"],
    blurb: "Led migration from Ektron CMS to WordPress with Jenzabar sync and WCAG-compliant rebuild.",
    color: "var(--accent-blue)",
  },
  {
    name: "SCUHS",
    stack: ["WordPress", "PHP", "Security"],
    blurb: "Hardened a compromised university site — remediation, monitoring, and long-term protection.",
    color: "var(--accent-red)",
  },
  {
    name: "Bien Leaf",
    stack: ["Shopify"],
    blurb: "Launched a Shopify storefront for a tea brand with custom product pages and catalog setup.",
    color: "var(--accent-green)",
  },
  {
    name: "Schreiner University",
    stack: ["WordPress"],
    blurb: "Ongoing maintenance, content updates, and bug fixes for a Texas-based university site.",
    color: "var(--accent-yellow)",
  },
  {
    name: "DineCollege",
    stack: ["WordPress"],
    blurb: "Contributed to a redesign with responsive layouts and measurable performance gains.",
    color: "var(--accent-blue)",
  },
];
...
function Hero() {
  return (
    <section id="top" className="relative overflow-hidden border-b border-ink bg-grid">
        ...
        <div className="md:col-span-8">
          <div className="mb-6 inline-flex items-center gap-2 border-2 border-ink bg-card px-3 py-1 font-mono text-xs shadow-brutal">
            <span className="relative flex h-2 w-2">
              ...
              <span className="relative inline-flex h-2 w-2 rounded-full" style={{ background: "var(--accent-green)" }} />
            </span>
          ...
          <div className="mt-10 flex flex-wrap gap-3">
            <a href="#projects" className="group inline-flex items-center gap-2 border-2 border-ink bg-foreground px-5 py-3 font-mono text-sm font-bold text-background shadow-brutal transition-transform hover:-translate-y-0.5 hover:translate-x-0 hover:shadow-brutal-lg">
              See projects <span className="transition-transform group-hover:translate-x-1">→</span>
            </a>
            <a href="#contact" className="inline-flex items-center gap-2 border-2 border-ink bg-card px-5 py-3 font-mono text-sm font-bold shadow-brutal transition-transform hover:-translate-y-0.5">
              Get in touch
            </a>
          </div>
        </div>
  ...
  return (
    <section id="about" className="border-b border-ink">
      <div className="mx-auto max-w-7xl px-5 py-20 md:py-28">
        <SectionHeader tag="01 — about" title="The TL;DR" sub="Frontend-leaning full-stack. WordPress whisperer. Spring learner. SEO past life." />
        <div className="grid gap-6 md:grid-cols-3">
          <Card title="Build" body="Responsive UIs, RESTful services, and CMS-driven sites that hold up in production." />
          <Card title="Migrate" body="Hosting moves, CMS replatforms (Ektron→WordPress), zero-downtime cutovers." />
          <Card title="Optimize" body="Perf budgets, asset & caching strategy, accessibility audits, security hardening." />
        </div>
      </div>
    </section>
  );
...
function Card({ title, body }: { title: string; body: string }) {
  return (
    <div className="group relative border-2 border-ink bg-card p-6 shadow-brutal transition-transform hover:-translate-y-1 hover:shadow-brutal-lg">
      <div className="mb-4 inline-block border-2 border-ink px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider">{title}</div>
...
function Experience() {
  return (
    <section id="work" className="border-b border-ink bg-card/40">
      <div className="mx-auto max-w-7xl px-5 py-20 md:py-28">
        <SectionHeader tag="02 — experience" title="Where I've been shipping." />
        <div className="border-2 border-ink bg-card shadow-brutal-lg">
          {EXPERIENCE.map((e, i) => (
            <article
              key={e.company}
            ...
            >
              <div className="md:col-span-3">
                ...
                <div className="mt-1 font-mono text-xs text-muted-foreground">{e.location}</div>
                {e.tag === "current" ? (
                  <span className="mt-3 inline-block border-2 border-ink px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider" style={{ background: "var(--accent-yellow)" }}>
                    now
                  </span>
                ) : null}
              </div>
...
function Projects() {
  return (
    <section id="projects" className="border-b border-ink">
      <div className="mx-auto max-w-7xl px-5 py-20 md:py-28">
        <SectionHeader tag="03 — projects" title="Selected work." sub="Production projects across MERN, WordPress, and Shopify." />
        <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
          {PROJECTS.map((p) => (
            <article key={p.name} className="group relative flex flex-col border-2 border-ink bg-card shadow-brutal transition-transform hover:-translate-y-1 hover:shadow-brutal-lg">
              <div className="h-32 border-b-2 border-ink" style={{ background: p.color }}>
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                <p className="mt-2 flex-1 text-sm text-foreground/80">{p.blurb}</p>
                <div className="mt-4 flex flex-wrap gap-1.5">
                  {p.stack.map((s) => (
                    <span key={s} className="border border-ink px-2 py-0.5 font-mono text-[10px] uppercase tracking-wider">
                      {s}
                    </span>
                  ))}
                </div>
...
function Stack() {
  return (
    <section id="stack" className="border-b border-ink bg-card/40">
      <div className="mx-auto max-w-7xl px-5 py-20 md:py-28">
        <SectionHeader tag="04 — stack" title="Tools of the trade." />
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          {Object.entries(STACK).map(([k, v]) => (
            <div key={k} className="border-2 border-ink bg-card p-5 shadow-brutal">
              <div className="mb-3 font-mono text-[10px] uppercase tracking-[0.2em] text-muted-foreground">{k}</div>
              <div className="flex flex-wrap gap-1.5">
                {v.map((t) => (
                  <span key={t} className="border-2 border-ink px-2 py-1 font-mono text-xs">
                    {t}
                  </span>
                ))}
              </div>
    ...
    <section id="contact" className="border-b border-ink">
      <div className="mx-auto max-w-7xl px-5 py-20 md:py-28">
        <div className="border-2 border-ink bg-foreground p-8 text-background shadow-brutal-lg md:p-16">
          <div className="font-mono text-xs uppercase tracking-[0.2em] opacity-60">05 — contact</div>
          <h2 className="mt-4 font-display text-4xl font-extrabold leading-[1] tracking-tight md:text-7xl">
            Got a project?
            <br />
            <span style={{ color: "var(--accent-yellow)" }}>Let&apos;s build it.</span>
          </h2>
          <p className="mt-6 max-w-xl text-background/70 md:text-lg">
            Open to full-time roles and select freelance engagements. Fastest reply via email.
          </p>
          <div className="mt-10 grid gap-3 md:grid-cols-3">
            <ContactLink label="email" value="Gouravbasera06@gmail.com" href="mailto:Gouravbasera06@gmail.com" />
            <ContactLink label="phone" value="+91 70425 74559" href="tel:+917042574559" />
            <ContactLink label="linkedin" value="/in/gourav-basera" href="https://www.linkedin.com/in/gourav-basera/" />
          </div>
        </div>
      </div>
    </section>
...
function ContactLink({ label, value, href }: { label: string; value: string; href: string }) {
  return (
    ...
    >
      ...
      <span className="truncate font-mono text-sm">{value} <span className="opacity-60 transition-transform group-hover:translate-x-1 inline-block">↗</span></span>
    </a>
  );
}
    ...
    <footer className="bg-background">
      <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-3 px-5 py-8 md:flex-row md:items-center">
        <div className="font-mono text-xs text-muted-foreground">
          © {new Date().getFullYear()} Gourav Basera · built with care in Gurugram
        </div>
        <div className="font-mono text-xs text-muted-foreground">
          v1.0 · last_deploy: <span className="text-foreground">{new Date().toISOString().slice(0, 10)}</span>
        </div>
      </div>
    </footer>
  );
